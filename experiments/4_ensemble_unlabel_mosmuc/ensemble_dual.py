...
paraDict = {
        ...
        "trainData": "munich", # training data could be the training data of LCZ42 data, or data of one of the cultural-10 city
        "testData": "moscow",  # testing data could be all the data of the cultural-10 cities, or one of them.
        
        "normalization":"cms", # "ms": mean-std normalization, patch-wise
        "datFlag":2, # data selection: sentinel-1, sentinel-2, or both
        
        ### model name
        "modelName":'dual_streams_ensemble', # model name
        }
...
'''
modelPath = os.path.join(outcomeDir,'student1_model')
...
confusion_matrix,oa,aa,ka,pa,ua = modelOperDataLoader.confusionMatrix(predModel, cudaNow, target_data_loader, classification_loss)
...
'''
modelPath = os.path.join(outcomeDir,'student2_model')
predModel = resnetModel.resnet18(pretrained=False, inChannel=trainDataSet.nbChannel()).to(cudaNow)
predModel.load_state_dict(torch.load(modelPath,map_location=cudaNow))
confusion_matrix,oa,aa,ka,pa,ua = modelOperDataLoader.confusionMatrix(predModel, cudaNow, target_data_loader, classification_loss)
