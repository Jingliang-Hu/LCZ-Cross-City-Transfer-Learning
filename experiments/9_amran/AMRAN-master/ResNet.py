...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.conv1 = nn.Conv2d(10, 64, kernel_size=7, stride=1, padding=3, bias=False)
...
class AMRANNet(nn.Module):
    def __init__(self, num_classes=31):
        ...
        self.adjustSize = nn.Conv2d(2048,2048,2)
        self.Inception = InceptionV2(2048)
        self.avgpool = nn.AvgPool2d(7, stride=1)

    def forward(self, source, target, s_label, mu):
        source = self.adjustSize(self.sharedNet(source))
        target = self.adjustSize(self.sharedNet(target))
...
class InceptionV2(nn.Module):
    ...
    def __init__(self, in_channels, num_classes=17, bottle_channel=256):
